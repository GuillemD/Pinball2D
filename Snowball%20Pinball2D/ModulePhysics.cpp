...
update_status ModulePhysics::PostUpdate()
{
	...
	for(b2Body* b = world->GetBodyList(); b; b = b->GetNext())
	{
		for(b2Fixture* f = b->GetFixtureList(); f; f = f->GetNext())
		{
			...
			if (App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_DOWN && checked == false)
			{
				if (f->GetShape()->TestPoint(b->GetTransform(), mousePosition))
				{
					clickedBody.body = b;
					checked = true;
				}
				else
					clickedBody.body = NULL;
			}
		}
	}
	...
	{
		//set mouse joint target
		mouse_joint->SetTarget(mousePosition);
		//render line
		App->renderer->DrawLine(METERS_TO_PIXELS(mouse_joint->GetBodyB()->GetPosition().x), METERS_TO_PIXELS(mouse_joint->GetBodyB()->GetPosition().y), METERS_TO_PIXELS(mousePosition.x), METERS_TO_PIXELS(mousePosition.y),233,45,5);
